...
void Entity::glow(Memory& mem,Vector Color)
{
    ...
    mem.Write<glowMode>(ptr + offsets::GLOW_TYPE, { 2, 108, 40, 96 }); //Glow Mode
}
...
void WeaponXEntity::update(Memory& mem)
{
	...
	mem.Read<float>(wep_entity + offsets::OFFSET_BULLET_SPEED, projectile_speed);
	mem.Read<float>(wep_entity + offsets::OFFSET_BULLET_SCALE, projectile_scale);
	mem.Read<float>(wep_entity + offsets::OFFSET_ZOOM_FOV, zoom_fov);
	
    mem.Read<int>(wep_entity + offsets::OFFSET_AMMO, ammo);
}
